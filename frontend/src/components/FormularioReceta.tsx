...
const FormularioReceta: React.FC<FormularioRecetaProps> = ({
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!validar()) {
      onToast('error', 'Por favor, complete todos los campos obligatorios y corrija los errores.');
  ...
  async function handleGuardarAjuste() {
    if (!recetaAjuste || !validar()) {
      onToast('error', 'Por favor, complete todos los campos obligatorios del ajuste.');
      window.scrollTo({ top: 0, behavior: 'smooth' });
      return;
    }
    setGuardando(true);
